...
- **URL**: `https://{domain_name}/portfolio/…`（例: `…/portfolio/1_asuka/`）。リポジトリルートをホストの `dist/portfolio` に同期し、コンテナはそれを `/usr/share/nginx/portfolio` にマウントします（`/usr/share/nginx/html` は frontend の :ro マウントのため別パス）。ファイル更新のみで反映され、**nginx reload は不要**です。
